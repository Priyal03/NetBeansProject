		...
		<div id="content-table-inner">
			...
			<div id="table-content">
				...
				<!--  end message-green -->
                                <%
                                    ...
                                    {
                                        tb=request.getAttribute("opt").toString();
                                        txt=request.getAttribute("txt").toString();
                                    }
                                    int r=5;
                                    int pg=1;
                                    int ind=0;
				...
				<tr>
                                    <th id="tha" align="center" class="table-header-check" ><input id='demo_box' class='css-checkbox' name='checkall' type='checkbox' onclick="check_all(document.form1.check,this);"/><label for='demo_box' name='demo_lbl' class='css-label'></label></th>
                                        <th class="table-header-repeat line-left"><a class="info-tooltip" title="<%=srt.equals("uid")?"Descending":"Ascending"%>" href="search?opt=<%=tb%>&txt=<%=txt%>&ind=<%=ind%>&r=<%=r%>&sort=uid&<%=srt.equals("uid")?"order=desc":"order=asc"%>">UID</a></th>
					<th class="table-header-repeat line-left minwidth-1"><a class="info-tooltip" title="<%=srt.equals("name")?"Descending":"Ascending"%>" href="search?opt=<%=tb%>&txt=<%=txt%>&ind=<%=ind%>&r=<%=r%>&sort=name&<%=srt.equals("name")?"order=desc":"order=asc"%>">Name</a>	</th>
					<th class="table-header-repeat line-left minwidth-1"><a class="info-tooltip" title="<%=srt.equals("user")?"Descending":"Ascending"%>" href="search?opt=<%=tb%>&txt=<%=txt%>&ind=<%=ind%>&r=<%=r%>&sort=user&<%=srt.equals("user")?"order=desc":"order=asc"%>">Username</a></th>
                                        <th class="table-header-repeat line-left minwidth-1"><a class="info-tooltip" title="<%=srt.equals("password")?"Descending":"Ascending"%>" href="search?opt=<%=tb%>&txt=<%=txt%>&ind=<%=ind%>&r=<%=r%>&sort=password&<%=srt.equals("password")?"order=desc":"order=asc"%>">Password</a></th>
					<th class="table-header-repeat line-left"><a class="info-tooltip" title="<%=srt.equals("gender")?"Descending":"Ascending"%>" href="search?opt=<%=tb%>&txt=<%=txt%>&ind=<%=ind%>&r=<%=r%>&sort=gender&<%=srt.equals("gender")?"order=desc":"order=asc"%>">Gender</a></th>
                                        <th class="table-header-repeat line-left minwidth-1"><a class="info-tooltip" title="<%=srt.equals("cityid")?"Descending":"Ascending"%>" href="search?opt=<%=tb%>&txt=<%=txt%>&ind=<%=ind%>&r=<%=r%>&sort=cityid&<%=srt.equals("cityid")?"order=desc":"order=asc"%>">City</a></th>
					<th class="table-header-repeat line-left minwidth-1"><a class="info-tooltip" title="<%=srt.equals("phone")?"Descending":"Ascending"%>" href="search?opt=<%=tb%>&txt=<%=txt%>&ind=<%=ind%>&r=<%=r%>&sort=phone&<%=srt.equals("phone")?"order=desc":"order=asc"%>">Phone</a></th>
                                        <th class="table-header-repeat line-left minwidth-1"><a class="info-tooltip" title="<%=srt.equals("dob")?"Descending":"Ascending"%>" href="search?opt=<%=tb%>&txt=<%=txt%>&ind=<%=ind%>&r=<%=r%>&sort=dob&<%=srt.equals("dob")?"order=desc":"order=asc"%>">Date of Birth</a></th>
                                        <th class="table-header-repeat line-left minwidth-1"><a class="info-tooltip" title="<%=srt.equals("email")?"Descending":"Ascending"%>" href="search?opt=<%=tb%>&txt=<%=txt%>&ind=<%=ind%>&r=<%=r%>&sort=email&<%=srt.equals("email")?"order=desc":"order=asc"%>">Email</a></th>
                                    ...
                                    if(res!=null)
                                    {
                                        for(int i=0;res.next();i++)
                                        {
                                            ...
                                            if(!al.contains(new Integer(uid)))
                                            {
                                                ...
                                                out.print("<td id=cty"+uid+">"+res.getInt(6)+"</td>");
			...
			<td>
                            <%
                                    ...
                                    if(ind>0)
                                        out.print("<a href='search?opt="+tb+"&txt="+txt+"&ind="+(ind-r)+"&r="+r+"' class='page-left'></a>");
                            %>
                                <div id="page-info">Page <strong><%=pg%></strong></div>
                            <%
                                    if(ind+r<t)
                                        out.print("<a href='search?opt="+tb+"&txt="+txt+"&ind="+(ind+r)+"&r="+r+"' class='page-right'></a>");
                            %>
			</td>
			</tr>
			</table>
