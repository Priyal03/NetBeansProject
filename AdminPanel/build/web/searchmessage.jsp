			...
			<div id="table-content">
				...
				<tr>
                                    <th id="tha" align="center" class="table-header-check" ><input id='demo_box' class='css-checkbox' name='checkall' type='checkbox' onclick="check_all(document.form2.check,this);"/><label for='demo_box' name='demo_lbl' class='css-label'></label></th>
                                        <th class="table-header-repeat line-left"><a class="info-tooltip" title="<%=srt.equals("mid")?"Descending":"Ascending"%>" href="search?opt=<%=tb%>&txt=<%=txt%>&ind=<%=ind%>&r=<%=r%>&sort=mid&<%=srt.equals("mid")?"order=desc":"order=asc"%>">MID</a></th>
					<th class="table-header-repeat line-left "><a class="info-tooltip" title="<%=srt.equals("sender")?"Descending":"Ascending"%>" href="search?opt=<%=tb%>&txt=<%=txt%>&ind=<%=ind%>&r=<%=r%>&sort=sender&<%=srt.equals("sender")?"order=desc":"order=asc"%>">From</a></th>
                                        <th class="table-header-repeat line-left "><a class="info-tooltip" title="<%=srt.equals("reciever")?"Descending":"Ascending"%>" href="search?opt=<%=tb%>&txt=<%=txt%>&ind=<%=ind%>&r=<%=r%>&sort=reciever&<%=srt.equals("reciever")?"order=desc":"order=asc"%>">To</a></th>
                                        <th class="table-header-repeat line-left "><a class="info-tooltip" title="<%=srt.equals("subject")?"Descending":"Ascending"%>" href="search?opt=<%=tb%>&txt=<%=txt%>&ind=<%=ind%>&r=<%=r%>&sort=subject&<%=srt.equals("subject")?"order=desc":"order=asc"%>">Subject</a></th>
					<th class="table-header-repeat line-left minwidth-1"><a class="info-tooltip" title="<%=srt.equals("msg")?"Descending":"Ascending"%>" href="search?opt=<%=tb%>&txt=<%=txt%>&ind=<%=ind%>&r=<%=r%>&sort=msg&<%=srt.equals("msg")?"order=desc":"order=asc"%>">Message</a></th>
                                ...
                                <%
                                    if(res!=null)
                                    {
                                        for(int i=0;res.next();i++)
                                        {
                                            ...
                                            out.print("<td id=from"+mid+">"+res.getInt("sender")+"</td>");
                                            out.print("<td id=to"+mid+">"+res.getInt("reciever")+"</td>");
